...
public class GetDistanceHandler: IRequestHandler<GetDistance, double>
{
    ...
    public async Task<double> Handle(GetDistance request, CancellationToken cancellationToken)
    {
        ...
        var airportInfoRequest1 = _teleportPlacesClient.GetAirportInfo(request.Code1);
        var airportInfoRequest2 = _teleportPlacesClient.GetAirportInfo(request.Code2);
        await Task.WhenAll(airportInfoRequest1, airportInfoRequest2);
        
        _logger.LogDebug("First: {@AirportInfo}", airportInfoRequest1.Result); 
        _logger.LogDebug("Second: {@AirportInfo}", airportInfoRequest2.Result); 
